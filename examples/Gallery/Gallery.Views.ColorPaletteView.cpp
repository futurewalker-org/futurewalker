...
auto ColorPaletteView::Make() -> Shared<ColorPaletteView>
{
    return View::MakeDerived<ColorPaletteView>();
}

ColorPaletteView::ColorPaletteView(PassKey<View> key)
  : View(key)
{
}

auto ColorPaletteView::GetInitializeTime() const -> TimeInterval
{
    return _initializeTime;
}

auto ColorPaletteView::Initialize() -> void
{
    ...
    column->AddChild(MakePaletteView(u8"Primary", primaryColors));
    column->AddChild(MakePaletteView(u8"Secondary", secondaryColors));
    column->AddChild(MakePaletteView(u8"Tertiary", tertiaryColors));
    column->AddChild(MakePaletteView(u8"Error", errorColors));
    column->AddChild(MakePaletteView(u8"Neutral", neutralColors));
    column->AddChild(MakePaletteView(u8"Neutral Variant", neutralVariantColors));

    AddChildBack(column);

    auto end = std::chrono::steady_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - bgn);
    _initializeTime = TimeInterval::MakeFromMilliseconds(duration.count());
}
}
