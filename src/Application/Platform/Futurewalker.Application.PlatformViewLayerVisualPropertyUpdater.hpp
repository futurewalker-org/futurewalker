...
private:
    ...
    auto PopVisual() -> void;
    auto UpdateFragment(PlatformViewLayerId const id, PlatformViewLayerVisualRenderParams const& renderParams, Shared<Graphics::DisplayList> const& displayList, Vector2<Dp> const& displayListOffset) -> void;
...
private:
    ...
    auto InternalReplaceNextPushNodeFragment(Vector2<Dp> const& offset, Matrix3x3<Dp> const& transform, Rect<Dp> const& clipRect, Optional<Graphics::Path> const& clipPath, Float64 const& opacity) -> void;
    auto InternalReplaceNextPopNodeFragment() -> void;
    auto InternalReplaceNextDisplayListFragment(Shared<Graphics::DisplayList> const& displayList, Vector2<Dp> const& displayListOffset) -> void;
